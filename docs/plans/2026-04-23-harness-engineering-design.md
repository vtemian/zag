...
- **Reasoning effort as a first-class knob.** Hardcoded `effort:medium` in `src/providers/chatgpt.zig:175` ships as a regression risk; it needs to come from the model/agent config, surfaced through `zag.provider{...}` with sensible defaults.
