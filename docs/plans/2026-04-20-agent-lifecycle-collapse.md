...
- `EventOrchestrator.zig`:
  ...
  - Store spawn deps on the orchestrator itself (allocator, wake_write_fd, lua_engine, provider, registry already available on config; stash the three not already fields: provider, registry, wake_write_fd — lua_engine and allocator already live there).
