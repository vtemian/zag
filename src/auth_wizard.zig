...
fn renderConfigLua(
    ...
    try body.appendSlice(allocator,
        ...
        \\--
        \\-- Auto-loaded for you on every startup (no require needed):
        \\--   zag.builtin.*   slash commands, keymaps, default tools
        \\--   zag.layers.*    prompt layers (env, AGENTS.md walk-up)
        \\--   zag.jit.*       just-in-time prompt context (AGENTS.md attach)
        \\--   zag.loop.*      tool-call loop detection
        \\--   zag.compact.*   default lossy compaction strategy
        \\--   zag.prompt      per-model prompt pack dispatcher
        \\-- Drop a same-named file under ~/.config/zag/lua/ to override.
        \\
        \\
    );
