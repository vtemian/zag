...
/// renders. Agent panes own the conversation pair; scratch-backed
/// panes borrow their Buffer from `BufferRegistry` and leave the pair
/// null. Every read site of `runner` or `conversation` must tolerate
/// null so scratch panes do not crash code paths that were originally
/// written for a single pane kind.
pub const Pane = struct {
    ...
    /// registry. Owns the node tree, the per-conversation buffer
    /// registry, the session handle, and persistence state.
    ...
    /// non-focused panes do not get reinterpreted as Conversation
    ...
    /// handling first (e.g. Conversation's Ctrl+R thinking-toggle),
...
/// AgentRunner / Conversation is allocated
...
/// `Conversation.readText` using the compositor's theme so the
...
pub fn readPaneById(
    ...
    const conv_buf = Conversation.fromBuffer(buf);
...
/// Conversation is allocated; the pane is
...
pub fn splitFocusedWithSession(
    ...
    // One resident pane per session: if `id` already has a pane (root or a
    // backgrounded split), reattach/focus it instead of loading a second
    // copy from disk. A duplicate mints a fresh runner + Conversation that
    // read a frozen on-disk snapshot, divorcing the view from the session's
    // live in-flight stream and leaking a pane per switch.
    if (self.findSessionPane(id)) |pane| {
        return self.showSessionPane(pane);
    }
...
/// The on-disk session id this pane is bound to, or null for a
/// non-persisted (scratch / subagent) pane. Read off the Conversation's
/// SessionHandle so `root_pane` and `extra_panes` entries answer uniformly.
fn paneSessionId(pane: *const Pane) ?[]const u8 {
    const conv = pane.conversation orelse return null;
    const sh = conv.session_handle orelse return null;
    return sh.meta.idSlice();
}

/// Find the single resident pane bound to session `id`, scanning the root
/// pane and every non-subagent extra pane. Returns null when the session
/// has no pane yet (first open). Backs the one-pane-per-session invariant:
/// `splitFocusedWithSession` reattaches the match rather than loading a
/// duplicate that would read a frozen on-disk snapshot and divorce the
/// view from the session's live in-flight stream.
fn findSessionPane(self: *WindowManager, id: []const u8) ?*Pane {
    if (paneSessionId(&self.root_pane)) |sid| {
        if (std.mem.eql(u8, sid, id)) return &self.root_pane;
    }
    for (self.extra_panes.items) |entry| {
        if (entry.is_subagent_view) continue;
        if (paneSessionId(&entry.pane)) |sid| {
            if (std.mem.eql(u8, sid, id)) return &entry.pane;
        }
    }
    return null;
}

/// Bring an already-resident session `pane` to the foreground. If its leaf
/// is still live (the pane is currently displayed) this is a focus; if the
/// pane was backgrounded (its leaf closed on a prior switch, handle now
/// stale) its existing surface is re-tiled. Either way the live runner and
/// Conversation are preserved, so the foreground view reconnects to the
/// session's in-flight stream.
fn showSessionPane(self: *WindowManager, pane: *Pane) !NodeRegistry.Handle {
    if (pane.handle) |h| {
        if (self.node_registry.resolve(h)) |node| {
            if (node.* == .leaf) {
                try self.focusById(h);
                return h;
            }
        } else |_| {
            // StaleNode: the pane was backgrounded; fall through to re-tile.
        }
    }
    return self.attachPaneAsSplit(pane);
}
...
/// null. Non-agent panes (those without a Conversation) always
...
pub fn attachSession(self: *WindowManager, pane: Pane) ?*Session.SessionHandle {
    const conversation = pane.conversation orelse return null;
...
/// Restore a pane from an on-disk session: rebuilds the conversation
/// tree from the session's JSONL entries, attaches the session handle,
/// and copies the stored session name (if any) back onto the
/// conversation. The runner's wire-format projection re-derives off
/// the tree at the next `submit`, so there is no parallel message list
/// to seed here.
...
fn autoNameSession(self: *WindowManager, pane: Pane) void {
    _ = self;
    const conversation = pane.conversation orelse return;
...
test "Pane composes conversation + runner" {
    const allocator = std.testing.allocator;

    const view = try allocator.create(Conversation);
    view.* = try Conversation.init(allocator, 0, "pane-test");
    defer {
        view.deinit();
    ...
    runner.* = AgentRunner.init(allocator, TestNullSink.sink(), view);
    defer {
        runner.deinit();
    ...
    // Both objects are reachable through the Pane. The runner holds
    // the conversation directly; the view lives on the pane.
    try std.testing.expectEqual(view, pane.conversation.?);
    try std.testing.expectEqual(runner, pane.runner.?);
    try std.testing.expectEqual(view, pane.runner.?.conversation);
...
test "Pane draft starts empty and grows via appendToDraft" {
    var view = try Conversation.init(std.testing.allocator, 0, "p");
    defer view.deinit();
...
test "Pane.handleKey delegates to buffer for buffer-internal chords (Ctrl+R)" {
    // Ctrl+R is Conversation's thinking-toggle: Pane.handleKey must
...
test "extra pane viewport is attached to its buffer" {
    ...
    // Each PaneEntry lives at a stable heap address, so its inline
    // viewport pointer is stable too. `viewportFor` collapses to
    // `&pane.viewport` for both root and extras; verify the round-trip.
    try std.testing.expectEqual(&entry.pane.viewport, wm.viewportFor(&entry.pane));
}
...
test "multiple splits maintain stable viewport pointers" {
    ...
    // First split: capture the address of the heap-allocated PaneEntry
    // and its inline viewport.
    _ = try wm.createSplitPane();
    const pane1_entry = wm.extra_panes.items[0];
    const pane1_inline_vp = &pane1_entry.pane.viewport;

    // Second split may relocate `extra_panes.items` (the ArrayList of
    // pointers grows). Because each PaneEntry is heap-allocated at a
    // stable address, the first pane's address (and therefore the
    // inline `viewport` pointer) must survive the reallocation.
    _ = try wm.createSplitPane();
    try std.testing.expectEqual(pane1_entry, wm.extra_panes.items[0]);
    try std.testing.expectEqual(pane1_inline_vp, &wm.extra_panes.items[0].pane.viewport);
}
...
test "zag.layout.split keeps legacy {buffer = {type = \"conversation\"}} form working" {
    ...
    // Legacy table form: the new pane gets a fresh Conversation
...
test "restorePane rebuilds both tree and messages" {
    ...
    var cb = try Conversation.init(allocator, 0, "restored");
    defer cb.deinit();
    var runner = AgentRunner.init(allocator, TestNullSink.sink(), &cb);
    defer runner.deinit();

    const pane: Pane = .{ .buffer = cb.buf(), .view = cb.view(), .conversation = &cb, .runner = &runner };
    try restorePane(pane, &handle, allocator);

    // The tree picks up both turns from the on-disk JSONL.
    try std.testing.expectEqual(@as(usize, 2), cb.tree.root_children.items.len);
    try std.testing.expectEqual(Conversation.NodeType.user_message, cb.tree.root_children.items[0].node_type);
    try std.testing.expectEqual(Conversation.NodeType.assistant_text, cb.tree.root_children.items[1].node_type);

    // Wire-format projection alternates user/assistant.
    var arena: std.heap.ArenaAllocator = .init(allocator);
    defer arena.deinit();
    var wire = try cb.toWireMessages(arena.allocator());
    defer wire.deinit(arena.allocator());
    try std.testing.expectEqual(@as(usize, 2), wire.items.len);
    try std.testing.expectEqual(types.Role.user, wire.items[0].role);
    try std.testing.expectEqual(types.Role.assistant, wire.items[1].role);
...
fn buildPickerFixture(allocator: std.mem.Allocator, f: *PickerFixture) !void {
    ...
    f.conversation = try Conversation.init(allocator, 0, "root");
    f.runner = AgentRunner.init(allocator, TestNullSink.sink(), &f.conversation);
    f.layout = Layout.init(allocator);
    ...
    f.wm = .{
        ...
        .layout = &f.layout,
        .compositor = undefined,
        .root_pane = .{ .buffer = f.conversation.buf(), .view = f.conversation.view(), .conversation = &f.conversation, .runner = &f.runner },
        .provider = &f.provider,
        .registry = &f.registry,
        .session_mgr = undefined,
        .lua_engine = null,
...
test "PaneDraftChange fires on float draft via zag.pane.set_draft" {
    ...
    var float_view = try Conversation.init(allocator, 99, "float");
...
const ModelPickerPluginFixture = struct {
    ...
    fn init(self: *ModelPickerPluginFixture, allocator: std.mem.Allocator) !void {
        ...
        self.wm.* = .{
            ...
            .root_pane = .{ .buffer = self.conversation.buf(), .view = self.conversation.view(), .conversation = &self.conversation, .runner = &self.runner },
...
test "re-opening a backgrounded session reattaches its pane instead of duplicating" {
    const allocator = std.testing.allocator;
    ...
    // First open: loads from disk, one extra pane.
    const handle1 = try wm.splitFocusedWithSession(session_id);
    try std.testing.expectEqual(@as(usize, 1), wm.extra_panes.items.len);
    const conv1 = wm.extra_panes.items[0].pane.conversation.?;

    // Background it: close its leaf. The PaneEntry + its runner stay
    // resident in extra_panes (nothing reaps them); only the layout leaf
    // goes away, mirroring what `_activate` does on a session switch.
    try wm.closeById(handle1, null);
    try std.testing.expectError(
        error.StaleNode,
        wm.node_registry.resolve(handle1),
    );
    try std.testing.expectEqual(@as(usize, 1), wm.extra_panes.items.len);

    // Re-open the same session: must REATTACH the existing pane, not load
    // a second copy from disk. No new PaneEntry, same Conversation pointer,
    // and the returned handle is a fresh live leaf.
    const handle2 = try wm.splitFocusedWithSession(session_id);
    try std.testing.expectEqual(@as(usize, 1), wm.extra_panes.items.len);
    try std.testing.expectEqual(conv1, wm.extra_panes.items[0].pane.conversation.?);
    const node2 = try wm.node_registry.resolve(handle2);
    try std.testing.expect(node2.* == .leaf);
    try std.testing.expectEqual(node2, wm.layout.focused);
}

test "re-opening the currently displayed session focuses it without duplicating" {
    const allocator = std.testing.allocator;
    ...
    // Re-open while still displayed: focuses the live leaf, no new pane,
    // and returns the same handle (the leaf was never closed).
    const handle2 = try wm.splitFocusedWithSession(session_id);
    try std.testing.expectEqual(@as(usize, 1), wm.extra_panes.items.len);
    try std.testing.expectEqual(handle1, handle2);
}

test "opening the root pane's session focuses root without an extra pane" {
    const allocator = std.testing.allocator;
    ...
    // Build the root pane's session and attach it to the root Conversation,
    // mirroring main.zig's `root_buffer.attachSession(sh)`.
    const root_sh = try allocator.create(Session.SessionHandle);
    defer allocator.destroy(root_sh);
    root_sh.* = try mgr_opt.?.createSession("test-model");
    defer root_sh.close();
    var root_id_buf: [32]u8 = undefined;
    @memcpy(root_id_buf[0..root_sh.id_len], root_sh.id[0..root_sh.id_len]);
    const root_id = root_id_buf[0..root_sh.id_len];
    ...
    // Opening the root session must reuse root_pane: no extra pane appears.
    const handle = try wm.splitFocusedWithSession(root_id);
    try std.testing.expectEqual(@as(usize, 0), wm.extra_panes.items.len);
