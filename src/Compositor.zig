    ...
    switch (node.*) {
        .leaf => |leaf| {
            if (leaf.viewport.isDirty(leaf.buffer.contentVersion())) {
                ...
                // Mirror the prompt-row reservation in drawBufferContent
                // so content-dirty clears don't wipe the per-pane prompt
                // (when one is present). The work_row stays owned by
                // `drawPanePrompt` regardless of agent state, so
                // `reserve` only depends on pane geometry now.
                const has_prompt = leafHasPrompt(leaf_drafts, &leaf);
...
/// the corresponding NodeLineCache entries so they don't outlive the
/// registry-owned TextBuffer bytes their spans borrow into. On overflow (more than
...
fn drawBufferIntoRect(
    ...
    // Reserve a fixed two-row footer for the prompt + working-line
    // slot when the pane has a prompt and is tall enough. The slot
    // belongs to `drawPanePrompt` even when no agent is running so
    // the row is always managed (cleared / repainted) by that pass.
    // Coupling reserve to `agent_running` caused a ghost: when the
    // agent finished, the work row was no longer cleared by
    // `drawPanePrompt`, content paint didn't repaint until something
    // else marked the leaf dirty, and stale `* Working… (Ns)` text
    // lingered on screen across idle frames.
    ...
    // text are cache-owned or borrowed into the registry-owned TextBuffer
    // bytes; the ArrayList header and the slice both live on the per-frame arena.
...
test "composite skips clean buffer leaves" {
    ...
    const pad_h = theme.spacing.padding_h;
    // The 2-col user marker gutter precedes the content, so "hello" starts at
    // 1+pad_h+2; the third char 'l' lands at 1+pad_h+4.
    try std.testing.expectEqual(@as(u21, 'l'), screen.getCellConst(1, 1 + pad_h + 4).codepoint);

    // Manually overwrite a cell to detect if the leaf is redrawn.
    screen.getCell(1, 1 + pad_h + 4).codepoint = 'Z';
    ...
    try std.testing.expectEqual(@as(u21, 'Z'), screen.getCellConst(1, 1 + pad_h + 4).codepoint);
}
...
test "scratch leaf still renders correctly with a float overhead" {
    ...
    // Float in the lower-right; the tile content must still resolve to the
    // user message. With the 2-col user marker gutter, "hello" starts at
    // 1+pad_h+2, so the third char 'l' lands at 1+pad_h+4.
...
test "planScroll: long line wraps and scroll math is in physical rows" {
    // A root user_message carries a 2-col marker gutter on its first physical
    // row (continuations restart at content_x with no re-indent). At width 20
    // the first row holds gutter(2) + 18 content chars, then the remaining
    // 42 chars wrap to 3 more rows (20 + 20 + 2): total 4 physical rows. The
    // gutter cost is applied by Conversation's gutter-aware metrics passes.
    const allocator = std.testing.allocator;
    var cb = try @import("Conversation.zig").init(allocator, 0, "test");
    defer cb.deinit();
...
test "planScroll: scrolling past the wrapped tail keeps recent rows visible" {
    ...
    const theme = Theme.defaultTheme();
    // 10 user_messages with distinct content "line0".."line9" (5 chars). At
    // width 5 each line carries a 2-col marker gutter on its first physical
    // row: row 1 holds gutter(2) + 3 content chars, row 2 holds the last 2
    // chars, so each node wraps to 2 physical rows -> total_rows = 20.
    // visible_rows = 6, scroll = 0 -> visible_start_rows = 14. Walking nodes
    // of 2 rows each, cum reaches 14 exactly after lines 0-6 (7 nodes x 2
    // rows). Line 7 is the first where cum + rows > visible_start_rows, with
    // leading_skip_rows = 14 - 14 = 0. The window holds lines 7-9 (6 rows).
    //
...
// tests below. Emits the exact 2-span shape requested in the node's
// TextBuffer-resolved bytes so the test can craft a logical line whose
// wrap boundary lands at a span seam (Bug A) or in the middle of a
// non-first span (Bug B). Span text borrows from the registry-owned
// TextBuffer bytes, which outlive the rendered line via `NodeLineCache`.
...
test "composite: multi-span wrap doesn't drop content (Bug A regression)" {
    ...
    // pane_width=5. A `.custom` node gets a 2-col blank (`.none` marker)
    // gutter drawn first at content_x: cols 2,3 are the gutter, so "abcde"
    // begins at col 4. 'a'4,'b'5,'c'6 fill the row (col 7 == max_col → wrap),
    ...
    try std.testing.expectEqual(@as(u21, 'c'), screen.getCellConst(1, 6).codepoint);
...
test "composite: wrapped continuation lands at content_x, not span tail (Bug B regression)" {
    ...
    // pane_width=11. A `.custom` node gets a 2-col blank (`.none` marker)
    // gutter drawn first at content_x (cols 2,3), so the content starts at
    // col 4. "hello ": 'h'4,'e'5,'l'6,'l'7,'o'8,' '9. Then "world from beyond":
    // 'w'10,'o'11,'r'12 fill out the row (col 13 == max_col on the next
    ...
    try std.testing.expectEqual(@as(u21, 'r'), screen.getCellConst(1, 12).codepoint);
    // Wrapped continuation lands at col 2 (content_x), NOT re-indented by the
    // gutter. The buggy code put it at the span tail.
    try std.testing.expectEqual(@as(u21, 'f'), screen.getCellConst(2, 5).codepoint);
    try std.testing.expectEqual(@as(u21, 'r'), screen.getCellConst(2, 6).codepoint);
    // Cols 0 and 1 (outside content area) must be blank.
...
test "composite: bottom-anchored mid-line scroll keeps tail visible (Bug C regression)" {
    ...
    //   - Buffer has 5 user_messages "12345678". Each renders with a 2-col
    //     `\u{203A} ` marker gutter on its first physical row + "12345678"
    //     (8 cells). At pane_width=5 the first row holds gutter(2)+"123" and
    //     the continuation (restarting at content_x, no gutter) holds
    //     "45678", so each line wraps to 2 physical rows -> total_rows=10.
    ...
    // = outer.height - 2 - reserve_prompt(2) >= 3 -> outer.height >= 7.
    // The reserve is now 2 unconditionally for prompt-bearing panes
    // (the prompt row + the always-reserved work_row slot above it).
    var screen = try Screen.init(allocator, 8, 7);
    defer screen.deinit();
    const theme = Theme.defaultTheme();
    ...
    const outer = Layout.Rect{ .x = 0, .y = 0, .width = 8, .height = 7 };
    compositor.drawBufferIntoRect(cb.view(), &viewport, outer, true);

    // The user marker gutter (2 cols) occupies the first physical row, so
    // "12345678" wraps to gutter+"123" / "45678". content_y = outer.y + 1 +
    // padding_v(0) = 1. visible_rows=3, so content rows occupy 1..3. The
    // bottom visible row is row 3, holding line 4's tail "45678" at cols 2..6.
...
test "work_row is reserved unconditionally so content never lands on it" {
    const allocator = std.testing.allocator;

    // 10-row, 30-col pane: interior height = 8, reserve = 2, content
    // height = 6, last content row at outer.y + 6 = outer.y + 6.
    ...
    // Fill the conversation with more lines than the interior holds so
    // the renderer scrolls to the tail and content reaches the bottom-
    // most legal row.
    const buf_id: u32 = 1;
    ...
    // Last content row: outer.height - 4 (frame[1] + prompt[1] +
    // work_row[1] reserved above it). Has text.
    ...
    // work_row: outer.height - 3. Blank — content never paints here.
...
test "working line is wiped on the frame after agent_running flips false" {
    // The ghost screenshot: agent finishes, `agent_running` flips to
    // false, but the prior frame's "Working… (Ns)" text stayed on
    // screen because drawPanePrompt's clearRect was inside the
    // running-only branch. Reserving work_row unconditionally and
    // moving the clearRect outside the agent_running gate is what
    // kills the ghost.
    const allocator = std.testing.allocator;
    ...
    // Build a layout with a single leaf bound to a conversation so
    // composite() can drive both the buffer paint and drawPanePrompt.
    const buf_id: u32 = 1;
    var cb = try @import("Conversation.zig").init(allocator, buf_id, "test");
    defer cb.deinit();
    ...
    var root_leaf: Layout.LayoutNode = .{ .leaf = .{
        .view = cb.view(),
        .viewport = &viewport,
        .buffer = cb.buf(),
        .rect = outer,
    } };
    // Hand-construct Layout: root is stack-allocated so we cannot
    // call `Layout.deinit` (it would try to `destroyNode` a non-heap
    // pointer). The floats list defaults to empty; the focused-float
    // slot defaults to null; the frame allocator stays nil.
    var layout: Layout = .{
        .root = &root_leaf,
        .focused = &root_leaf,
        .allocator = allocator,
    ...
    const draft: []const u8 = "";
    var drafts = [_]LeafDraft{.{
        .leaf = &root_leaf.leaf,
        .draft = draft,
        .draft_cursor = 0,
    }};

    // Frame 1: agent_running = true. Working line lands on work_row.
    compositor.layout_dirty = true;
    compositor.composite(&layout, drafts[0..], &.{}, .{
    ...
    // Frame 2: agent_running = false. With the fix, the work_row is
    // blanked by drawPanePrompt even though the leaf is no longer
    // dirty. Before the fix, "Working… (5s)" lingered.
    compositor.composite(&layout, drafts[0..], &.{}, .{
        .mode = Keymap.Mode.insert,
        .agent_running = false,
        .elapsed_ms = 0,
