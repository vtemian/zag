...
pub fn emitScenario(
    ...
    try writer.print("# went through, against the user's real ~/.config/zag/. Assistant output\n", .{});
    try writer.print("# comes from the live provider, so it won't match the original byte-for-byte.\n", .{});
    try writer.print("\n", .{});
